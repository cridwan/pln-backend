...
abstract class SubBidangCore extends MasterCore implements WithImportExcel
{
    #[DoNotDiscover]
    ...
    public function mapping($data, $index)
    {
        ...
        try {
            SubBidang::create([
                'name' => $data['name'],
                'bidang_uuid' => trim(str($data['bidang_uuid'])->explode('/')->toArray()[1]),
            ]);
        } catch (\Throwable $th) {
