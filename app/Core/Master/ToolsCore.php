...
abstract class ToolsCore extends MasterCore implements WithImportExcel
{
    #[DoNotDiscover]
    ...
    public function attributeExport(): array
    {
        return [
            ...
            new AttributeData('status', 'STATUS'),
    ...
    public function mapping($data, $index)
    {
        ...
        try {
            $globalUnit = str($data['global_unit_uuid'])->explode('/')->toArray();
            Tools::create([
                'name' => $data['name'] ?? '',
                'merk' => $data['merk'] ?? '',
                'price' => $data['price'] ?? '',
                'global_unit_uuid' => trim(end($globalUnit)),
                'status' => $data['status'] ?? ToolStatusEnum::GENERAL,
            ]);
        } catch (\Throwable $th) {
